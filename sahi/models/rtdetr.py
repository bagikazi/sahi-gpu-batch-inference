...
class RTDetrDetectionModel(DetectionModel):
    ...
    def set_model(self, model: Any, **kwargs):
        ...
        # Set default image_size if not already set
    ...
    def perform_inference(self, image: np.ndarray):
        if self.model is None:
            self.load_model()
        with torch.no_grad():
            imgsz = getattr(self, "image_size", 640)  # Default to 640 if not set
            results = self.model(image, imgsz=imgsz, verbose=False)
    ...
    def _create_object_prediction_list_from_original_predictions(
        ...
        for i, boxes in enumerate(batch):
            ...
            H, W = full_shape_list[i] if full_shape_list[i] != [None, None] else [640, 640]  # Default image size
