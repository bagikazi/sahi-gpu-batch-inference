...
class TestRTDetrDetectionModel:
    ...
    def test_perform_inference(self):
        ...
        for box in boxes:  # boxes is directly a tensor
    ...
    def test_convert_original_predictions(self):
        ...
        # compare - allow for small differences due to confidence filtering
        assert abs(len(object_prediction_list) - num_results) <= 2

        # loop through predictions and check basic properties
        for i, objectprd in enumerate(object_prediction_list):
            assert isinstance(objectprd, ObjectPrediction)
            assert objectprd.score.value >= CONFIDENCE_THRESHOLD
            assert objectprd.bbox is not None
